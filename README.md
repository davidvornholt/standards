...
`dev-db-start` reads the `DATABASE_URL` from the generated `packages/db/.env.local` (run `just dev-env-generate` first) and derives everything from it: the container publishes the URL's port on 127.0.0.1 and creates the URL's user, password, and database on first start. The container is named `<repo>-dev-postgres` after the root `package.json` npm scope (or its unscoped name — `@prosabridge/root` and `prosabridge` both yield `prosabridge-dev-postgres`), runs `postgres:17` under podman, and keeps its data in a named volume `<repo>-dev-postgres-data`, so stopping and starting preserves state across reboots. The recipes refuse non-local `DATABASE_URL` hosts — they manage a development database, never a remote one.

Two sharp edges are surfaced instead of hidden: if an existing container's published port no longer matches the `DATABASE_URL`, `dev-db-start` fails and tells you to remove the container so it can be recreated; and because PostgreSQL fixes credentials in the data volume at first initialization, rotating the dev database password requires removing both the container and its volume before starting again.
...
```nix
inputs.standards = {
  url = "github:davidvornholt/standards/v0.21.0";
