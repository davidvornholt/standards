...
A merge that inherits a version an earlier commit declared is a no-op: it reads the parent commit's manifest, finds the version unchanged, and stops without contacting npm. Reverting a release commit withdraws the declaration the same way. Either path still requires the declared version's `vX.Y.Z` tag to exist, so a release that never completed turns every later merge red instead of being skipped in silence.

Publishing only ever happens on the declaring commit, and only while that commit is still the tip of `main`. npm builds provenance from `GITHUB_SHA`, which for this `workflow_run` trigger is the tip of the default branch rather than the commit the run checked out; publishing once `main` has advanced would attest a tree the tarball does not contain. The workflow refuses that case rather than producing an incoherent attestation.

**If a release commit's `Standards` run fails**, re-run it promptly — the release completes only while that commit is still the tip of `main`. Once `main` moves on, or if the gate failure needs a fix-forward commit, the declared version is spent: declare a new version on current `main`. Re-running a release that reached npm but not the tag verifies the published package's provenance against the declaring commit before reconciling the tag and Release. A version behind npm latest, unverifiable provenance, a tag pointing at another commit, or an inherited version with no tag all fail closed.
