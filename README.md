...
- **GitHub settings** — create `.github/settings.local.json` (seeded on new repos; `{"repository":{},"rulesets":[]}` when you have nothing to add) and run `bun standards github --apply` once with admin `gh` auth. It converges the live repo onto squash-only merging at both the repository and default-branch ruleset layers, plus the remaining declared settings and labels — including deleting hand-made rulesets that are not declared; extra live labels remain untouched. From then on `check` fails whenever merge commits, rebases, or any other declared live state drifts. If the repo is private on a GitHub plan that cannot enforce rulesets (Free, personal or organization), declare `"rulesetEnforcement": "unavailable-on-plan"` in the seam: the gate then skips rulesets and the plan-gated `allow_auto_merge` setting (GitHub accepts a PATCH for it with HTTP 200 and silently keeps it off), converges the remaining merge settings and labels, and prints an unprotected-branch notice on every run instead of failing forever or trusting state GitHub silently does not honor. For CI to verify all declared state, provision `ci.broker_app` in your SOPS-encrypted `secrets/ci.yaml` with `bun standards creds add github --dest ci:ci.broker_app` and install the broker App on the repository — CI mints a short-lived token from it with read-only "Administration" and "Issues" access, where Issues permits repository label reads and Administration is the broadest settings read a repository-scoped token can hold. Administration does not expose merge settings, which `check` reads over GraphQL, and it does not expose ruleset bypass actors to an installation token at any permission level: a declared-empty bypass list is verified through the GraphQL bypass-actor count, while naming the actors behind a declared non-empty list still needs a local run with admin `gh` auth.
...
```nix
inputs.standards = {
  url = "github:davidvornholt/standards/v0.18.0";
...
### Breaking migration to brokered settings verification

The isolated `github-settings` job no longer reads a durable fine-grained PAT. It resolves `ci.broker_app` from the SOPS-encrypted `secrets/ci.yaml` and mints a broker installation token scoped to the current repository with read-only Administration and Issues, so the durable credential in a consumer's secrets is the App key that already had to be there for the weekly sync — one identity instead of two, and nothing settings-capable that outlives an hour. Adopt it in this order: add read-only Administration to the broker App on its settings page first (the manifest is read only at creation time, so an existing App needs the toggle, and `actions/create-github-app-token` silently issues a token without a permission the App does not hold rather than failing — so a skipped toggle produces a quiet under-permissioned token, not an error), sync the canonical workflow, confirm a `Standards` run resolves the broker and does not warn about falling back to the workflow token, then delete `ci.github_settings_read_token` from `secrets/ci.yaml` and from the repo-owned `secrets/ci.example.yaml`, and finally revoke the PAT at <https://github.com/settings/personal-access-tokens>. A repo that has not provisioned the broker App still runs the gate on the workflow token, which can read labels but fails closed on settings it cannot see. The same change raises the weekly `Standards sync` version guard to `MINIMUM_STANDARDS_VERSION: "0.18.0"` and pins the isolated gate to that published CLI, because the settings comparison now depends on the GraphQL bypass-actor fallback that only exists from 0.18.0; an older installation would otherwise run a gate that cannot verify `bypass_actors` at all. Upgrade the exact `@davidvornholt/standards` dependency and `bun.lock` to at least 0.18.0 before accepting the canonical workflows.
...
- **`check` is the CI gate.** It confirms every synced file still matches what `sync` last wrote (offline, hash-based), rejects the raw token formed by `biome-` + `ignore` anywhere in canonical files so they remain compatible with consumer lint configurations, fails closed when the lock is absent, runs `doctor` to verify the repo-owned extension seams, and runs `structure` to enforce the monorepo layout contract (workspace and root script shapes, internal versioning, package `exports`, tsconfig inheritance, and a11y wiring for explicit `*.a11y.ts` suites). Once `.github/settings.json` is synced it also compares the live GitHub repository against the declaration via the API and fails closed on drift and on declared state the token cannot see. In the canonical workflow only, the unprivileged quality step sets `STANDARDS_SKIP_GITHUB_CHECK=true` because an isolated `github-settings` job runs that live comparison through a broker installation token minted from `ci.broker_app` in the SOPS-encrypted `secrets/ci.yaml`, scoped to the current repository with read-only "Administration" and "Issues" access. The isolated job sparse-checks out only the encrypted secret, the canonical SOPS resolver, and declarative settings inputs, grants the fallback workflow token only `contents: read` and `issues: read`, runs a checksum-pinned published CLI outside repository-controlled package configuration, and unsets the durable App identity before that CLI runs. Local `bun run check` remains fail-closed and performs the live comparison normally.
