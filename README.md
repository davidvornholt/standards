...
Version 0.11.0 adds support for `labels` declarations in `.github/settings.json` and `.github/settings.local.json` and introduces the host-level fix poller. The initial release does not yet add labels to the canonical declaration; that activation follows publication so consumers never receive a schema their installed CLI cannot parse. The poller is declarative-only: use `poller --print-units --config <path>` as input to the polling host's infrastructure repository, and let that owner provide the service identity, PATH, token environment, lingering, and deployment.
