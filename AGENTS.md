...
- Zero-install preconditions and the published standalone bootstrap CLI are the only exceptions to the Effect rules below. The complete allowed boundary is the canonical `.github/actions/standards-sync-preflight` action and its dependency-free helper closure, `packages/standards-release/scripts/classify-release.ts` and its dependency-free helper closure, and the published `packages/standards-cli/src/cli.ts` executable and its built-in-only helper closure listed in the package `files` allowlist. These boundaries may use explicit result values, raw Promises, and boundary throws because they run before repository or consumer dependencies are installed; they must not import third-party packages. The action may only decide whether setup should proceed, the classifier may only classify a release declaration, and the published CLI may only bootstrap, synchronize, validate, and reconcile declarative GitHub settings using Bun and Node built-ins. All code outside those enumerated boundaries, including release packing, registry inspection, publishing decisions, and GitHub Release reconciliation, follows the Effect and tagged-error rules below.
