...
- The complete canonical Effect exception boundary consists of the zero-install `.github/actions/standards-sync-preflight` action and its dependency-free generated helper closure, and the published standalone `packages/standards-cli/src/cli.ts` executable and its built-in-only helper closure listed in the package `files` allowlist. Source-repository-specific extensions belong in `AGENTS.local.md`; they are not part of the synced consumer contract. The canonical boundaries may use explicit result values, raw Promises, and boundary throws because they run before repository or consumer dependencies are installed; they must not import third-party packages. The action may only decide whether setup should proceed, and the published CLI may only bootstrap, synchronize, validate, and reconcile declarative GitHub settings using Bun and Node built-ins. All code outside the canonical boundary and any explicitly declared local extension follows the Effect and tagged-error rules below.
