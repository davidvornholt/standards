...
- **Merge-time protection is declared, not clicked.** The default-branch ruleset and repo merge settings live in canonical `.github/settings.json` (extended by `.github/settings.local.json`); `standards check` fails on drift and `bun standards github --apply` converges the live repo. Do not hand-edit rulesets in the GitHub UI or duplicate them in workflow logic.
