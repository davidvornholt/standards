...
| `.github/workflows/standards-sync.yml` | `sync-standards.local.json` configures it (optional `autoSync` opt-out for the weekly run, optional `ref` pin honored by the workflow and by CLI `init`/`sync`) |
