...
`github --check` verifies the live GitHub repository (merge settings, rulesets, and environments) against the merged declaration and fails closed on drift or API errors; `github --apply` converges the live repository — creating, updating, and deleting rulesets and deployment branch policies to exactly their declared sets. The canonical declaration owns the protected `standards-sync` environment with an exact `main`-only deployment policy. Apply never reads or writes secret values: separately store the workflow PAT as the environment secret `STANDARDS_SYNC_ENVIRONMENT_TOKEN`. Apply needs admin auth (the local `gh` CLI or `GH_TOKEN`), which CI's token cannot hold, so drift found in CI is fixed by a human or local agent running `bun standards github --apply`. Never hand-edit rulesets, environments, deployment policies, or merge settings in the GitHub UI; change the declaration instead.
