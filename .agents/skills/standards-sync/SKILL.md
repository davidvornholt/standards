...
The five plane fields are one fail-closed contract. `environment` names a GitHub environment configured for only the exact `main` branch. `ageKeySecret` names its purpose-specific private key. `secretTarget` is a contained kebab-case basename other than `ci`. `brokerAppKey` is a purpose-scoped lowercase dotted path below `ci`. `ageRecipient` is the matching public identity. `recoveryAgeRecipients` is required whenever a plane is present and declares the personal identities allowed on every selected target. The structure gate requires the ciphertext metadata and exactly one matching `.sops.yaml` rule to contain exactly the selected plane recipient plus those recovery recipients, excluding legacy and sibling-plane recipients.
...
Private personal repositories need GitHub Pro; private organization repositories need Team or Enterprise. GitHub Free/private is an explicit adoption blocker: keep the legacy repository secret, upgrade, or separately review an external OIDC secret manager or trusted automation repository. Before opt-in, authenticate `gh` as a repository administrator and run `bun standards automation verify`. The strict, 30-day `sync-standards.environment-proof.json` binds immutable repo/owner IDs, policy digest, live plan capability, exact-main environment and branch-policy IDs, environment-secret presence, same-name repository-secret absence, observable organization-secret absence, and observed times. Regenerate it after admin changes. Once the initial proof is on `main`, record successful exact-main canonical runs with `--delivery automation=<run-id>` and `--delivery notifications=<run-id>`. The verifier binds workflow ID/path, main SHA/ref/event, successful secret-consuming steps, environment deployment, and a completion after capability observation. This boundary protects against contributor policy changes, not a malicious repository administrator.
