...
```json
{
  ...
  "recoveryAgeRecipients": ["age1..."]
}
```

The five plane fields are one fail-closed contract. `environment` names a GitHub environment configured for only the exact `main` branch. `ageKeySecret` names its purpose-specific private key. `secretTarget` is a contained kebab-case basename other than `ci`. `brokerAppKey` is a purpose-scoped lowercase dotted path below `ci`. `ageRecipient` is the matching public identity and must differ from every legacy CI recipient. `recoveryAgeRecipients` is required whenever a plane is present and declares the personal identities allowed on every selected target. The structure gate requires the ciphertext metadata and exactly one matching `.sops.yaml` rule to use age decryptors only and contain exactly the selected plane recipient plus those recovery recipients, excluding legacy, sibling-plane, PGP, KMS, Azure Key Vault, GCP KMS, and HashiCorp Vault recipients.

`Notify pause` has the independent shape `{ "notifications": { "environment": "notifications", "ageKeySecret": "NOTIFICATIONS_SOPS_AGE_KEY", "secretTarget": "notifications", "topicKey": "ntfy_topic_url", "ageRecipient": "age1..." } }`. Omitting either plane preserves its legacy behavior. If `secrets/ci.yaml` and its example remain, structure always validates their encryption and mirrored shape; both may be absent only after both secret-bearing planes are fully isolated and carry recorded successful delivery proof.

Private plan verification accepts exactly GitHub's `pro` name for a personal owner and `team`, `business`, or `enterprise` for an organization owner. GitHub Free/private or an unknown plan is an explicit adoption blocker: keep the legacy repository secret, upgrade, or separately review an external OIDC secret manager or trusted automation repository. Before opt-in, authenticate `gh` as a repository administrator and run `bun standards automation verify`. The strict, 30-day `sync-standards.environment-proof.json` binds immutable repo/owner IDs, policy digest, live plan capability, exact-main environment and branch-policy IDs, environment-secret presence, same-name repository-secret absence, observable same-name organization-secret absence, and observed times. A 403 or any other inability to inspect organization scope blocks adoption. Regenerate the proof after admin changes. Once the initial proof is on `main`, record successful exact-main canonical runs with `--delivery automation=<run-id>` and `--delivery notifications=<run-id>`. The verifier binds workflow ID/path, main SHA/ref/event, successful secret-consuming steps, environment deployment, and a completion after capability observation. This boundary protects against contributor policy changes, not a malicious repository administrator.
