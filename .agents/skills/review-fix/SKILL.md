...
- **defer**: real but outside the intent or below materiality. File a self-contained GitHub issue — evidence, concrete failure scenario, suggested verification, link to the PR — labeled `deferred-finding` (a canonical label from the declared GitHub settings). The label is the provenance marker downstream automation trusts: a maintainer approves a deferred issue for the fix poller by applying `approved-for-fix`. Deferral is the default for real-but-adjacent findings; "reproducible" is not "material".
