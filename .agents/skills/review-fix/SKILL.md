...
- **defer**: real but outside the intent or below materiality. File a self-contained GitHub issue — the plain-language opening AGENTS.md requires, then evidence, concrete failure scenario, suggested verification, link to the PR — labeled `deferred-finding`. Deferral is the default for real-but-adjacent findings; "reproducible" is not "material".
