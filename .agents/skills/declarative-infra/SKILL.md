...
- Provider credentials are written directly into SOPS targets by `bun standards creds`: Cloudflare tokens the broker minted are renewed and revoked by `plan` / `apply`, while GitHub App credentials let workflows mint short-lived installation tokens for selected repositories. A token the broker did not mint is never touched by `plan` / `apply`; they list it and print the exact `bun standards creds revoke --account <account-id> --token-id <id>` that retires it, which is how a leftover hand-made Cloudflare token is cleaned up. GitHub App keys rotate manually through the App settings page; do not claim `creds apply` rotates them, do not ask the operator to create brokered Cloudflare tokens by hand, and do not send them to the Cloudflare dashboard for a token `revoke` will take.
