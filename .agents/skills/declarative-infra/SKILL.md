...
- Trusted CI that builds and deploys NixOS closures uses a signed binary cache: validation publishes only after every Nix check passes, and deployment reads the validated closure with separate read-only credentials instead of rebuilding it. Leaving the cache out is a documented decision in the host repo; see `references/bootstrap.md`.
