...
That brokered rotation path is Cloudflare-only. GitHub App keys rotate manually: create a replacement private key on the App settings page, update every intended SOPS destination, verify workflows can mint installation tokens, and then revoke the old key on the settings page.
