...
- Plane separation: only `SOPS_AGE_KEY` bootstraps the machinery and stays a native Actions secret; every other CI secret lives in SOPS targets and is decrypted at runtime. The weekly Standards sync resolves `ci.broker_app.app_id` and `ci.broker_app.private_key`, then mints a short-lived installation token for only the current repository with Contents read and Pull requests write. Missing App credentials or failed minting stop the workflow; there is no durable sync token or workflow-token fallback.
