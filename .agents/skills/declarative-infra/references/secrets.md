...
Cloudflare API tokens and cross-repo GitHub credentials come from the credential broker rather than repeated provider-dashboard setup: `bun standards creds add cloudflare --dest <target>:<dotted.key> --permissions "<Account Group Name>"` mints a scoped, expiring account token and writes the value straight into the SOPS target (never into the terminal), while `bun standards creds add github --dest <target>:<dotted.key>` places the broker GitHub App's credentials for workflows to mint short-lived installation tokens at runtime. Cloudflare permission groups must match the account resource supplied by the command; zone-scoped groups such as DNS Write are rejected until a zone resource is supported. Install the GitHub App only on the selected repositories that need it. `bun standards creds plan` / `apply` reconcile Cloudflare tokens only: deleting the secret key from the SOPS file and applying revokes the provider-side token, while an expiring token is renewed by creating a replacement, durably writing and verifying its value, and then revoking the old token. The two reconciliation sources are the SOPS key structure and deterministically named provider tokens; desired-policy drift is not detected, and a repository rename or transfer is a visible re-mint-and-revoke event. Do not ask the operator to create brokered Cloudflare tokens by hand; the one-time machine setup is `standards creds login github` and `standards creds login cloudflare`.
