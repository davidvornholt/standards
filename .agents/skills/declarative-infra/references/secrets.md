...
- Plane separation: only `SOPS_AGE_KEY` bootstraps the machinery and stays a native Actions secret; every other CI secret, including workflow PATs like `ci.standards_sync_token` (repository-scoped with Pull requests write), lives in SOPS targets and is decrypted at runtime.
