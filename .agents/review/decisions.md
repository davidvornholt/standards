...
## GITHUB-SETTINGS-001: One read-only settings credential

The isolated GitHub settings job verifies through exactly one read-only credential carrying Administration and Issues access, never a second token-selection mechanism. Issues read exposes private issue content beyond the declared-label metadata the checker needs, and that read-only expansion is an accepted tradeoff: a private-repository probe on PR #94 confirmed that an Administration-plus-Metadata PAT can read merge settings through GraphQL but receives `FORBIDDEN` for `repository.labels`, so GraphQL offers no Metadata-only label path. That credential was originally a durable repository-scoped fine-grained PAT stored as `ci.github_settings_read_token`, accepted only while it stayed repository-scoped, SOPS-encrypted, isolated from repository-controlled executable code, and free of write permissions. PR #174 supersedes the durable half. GitHub exposes no API to create or revoke a PAT, so that token could never be rotated by `bun standards creds` and had to be minted and retired by hand; the job now mints a current-repository installation token from the `ci.broker_app` App key the weekly sync already required, leaving one durable GitHub identity instead of two and nothing settings-capable that outlives an hour. The single-credential shape and its Issues-read tradeoff carry over unchanged, and the fallback to the workflow token is deliberate, because a repo that has not provisioned the App has simply not adopted the broker yet while the gate still fails closed on state that token cannot see. Revisit this decision if GitHub changes the label permission boundary, gains a durable-token lifecycle API, or the settings gate comes to need a permission outside the broker App's ceiling.
