...
- **`GITHUB_REPOSITORY`** (release workflow only) — repository identifier supplied automatically by GitHub Actions. The internal release-state wrapper requires it together with `GITHUB_TOKEN`; local CLI commands do not consume it.
