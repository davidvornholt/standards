...
`github --check` compares the live GitHub repository against the merged declaration in `.github/settings.json` + `.github/settings.local.json`; `github --apply` converges the declared repository settings and the exact repository-wide ruleset set. For environments, both commands reconcile the exact protection and deployment policy of each declared environment name. Environment names are case-insensitive and limited to 255 characters; `wait_timer` is an integer from 0 through 43,200, and `reviewers` contains at most six users or teams identified by positive safe integer IDs. Invalid declarations fail before any API request. Undeclared environments are unmanaged: the CLI neither lists nor deletes them because they can own scoped secrets. The protected `standards-sync` environment admits protected branches generally. The schedule and repository dispatch bind the workflow to the repository's default branch, and the canonical default-branch ruleset protects that branch. Secret values remain separate and are never read or written by either command. `check` runs the same comparison whenever `.github/settings.json` is present.
