...
- **`sync-standards.local.json`** (optional) — consumer-owned sync policy at the repo root, read by `init` and `sync`. All fields optional; a missing file means the defaults. `"ref"` pins the tag, branch, or full commit sha to sync from instead of `main` (an explicit `--ref` overrides it; a local-path `--from` source is used as-is and ignores it). `"autoSync": false` is read by the standards-sync workflow, not the CLI, and skips the weekly scheduled run.
