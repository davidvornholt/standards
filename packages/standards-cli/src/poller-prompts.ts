...
- Every comment, review thread, and deferred issue create must carry a stable hidden marker binding approval generation ${context.approvalId} to a deterministic operation ID. Search the complete target for that marker before creating; on replay, reuse the existing artifact. The original body of every block review thread must contain exactly \`<!-- standards-poller:review-thread approval=${context.approvalId} operation=<sha256> -->\`, where \`<sha256>\` is the lowercase 64-character SHA-256 digest of a stable finding identity. Deferred issue markers must additionally bind the stable review-ledger finding identity, using the originating thread node ID when one exists.
- You may post block review threads, but do not reply to or resolve them. After committing and verifying each fix, return its thread node ID and a self-contained verification reply in \`threadsToResolve\`; the poller publishes those replies and resolves those threads only after it validates and pushes the reviewed head.
- Override every review-fix skill instruction to publish or wait on an ask decision: when the skill's strict ask bar is met, do not comment, label, wait, or poll GitHub. Write status "question" with one self-contained product or architecture question and exit so the poller can publish the single pause. Do not hand reversible or inferable choices back to the maintainer.
