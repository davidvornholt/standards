...
// enumerators so a new accepted spelling can never reach only one of them.
...
const assertFixedRunnerJobsDoNotUseConfigurableRunner = (
  ...
  if (violations.length > 0) {
    throw new Error(
      `Jobs must use a fixed runner, but these select vars.CI_RUNNER: ${violations.join(', ')}`,
    );
  }
};
...
const runWorkflowVersionGuard = (version: string): RunResult => {
  ...
  return runExecutable(
    ...
    { MINIMUM_STANDARDS_VERSION: '0.18.0' },
  );
};
...
describe('canonical standards workflow settings security', () => {
  it('isolates the settings comparison from repository-controlled executable code', () => {
    ...
    expect(installStep).toContain('standards_version=0.18.0');
    expect(installStep).toContain(
      'standards_sha=001e736ce8c87548c699877ee4322dbec773bf99f7d4f4f591ccfe80bd07420aceffc345bb840daaf7688d82174ee3da38a3c60c7f9d8a65cd7a58bfa5d02004',
    );
...
describe('canonical workflow runner boundaries', () => {
  ...
  it('rejects configurable runners in manifest-owned .yaml workflows', () => {
    ...
    expect(() =>
      ...
      'Jobs must use a fixed runner, but these select vars.CI_RUNNER: .github/workflows/additional-check.yaml:additional-check',
    );
  });
});
...
describe('standards sync workflow policy', () => {
  ...
  it('makes the 0.18.0 guard unconditional', () => {
    const workflow = readFileSync(SYNC_WORKFLOW, 'utf8');
    expect(workflow).toContain('MINIMUM_STANDARDS_VERSION: "0.18.0"');
