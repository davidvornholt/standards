...
const LABEL_VISIBILITY_PROBLEM =
  "declared labels not visible to this token, so the gate cannot verify them. In CI, grant the github-settings job's workflow token issues: read (or pull-requests: read); locally use a token with one of those permissions";
