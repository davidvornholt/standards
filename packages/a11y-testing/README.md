...
- `@davidvornholt/a11y-testing/axe` — `scanWcag22AaViolations(page)` runs Axe against the current page state for the full WCAG 2.2 AA tag set (`wcag2a`, `wcag2aa`, `wcag21a`, `wcag21aa`, `wcag22aa`, exported as `wcag22AaTags`) and returns the mapped violations. Specs assert `expect(await scanWcag22AaViolations(page)).toEqual([])` so every test carries its own visible zero-violation assertion.
